...
async function resetLead(snapshot: QueryDocumentSnapshot<DocumentData>, cutoff: number): Promise<string | null> {
...
export const processLeadInactivityDispatch = onSchedule("every 1 hours", async () => {
  ...
  const agencyIds = new Set(reallocated.filter((item) => item.agencyId).map((item) => item.agencyId as string));
  await Promise.all([...agencyIds].map(async (agencyId) => {
    const staff = await db.collection("users").where("agencyId", "==", agencyId).get();
    await Promise.all(staff.docs.filter((user) => ["ceo", "secretary", "secretariat"].includes(user.data().agencyRole) || user.data().role === "secretariat").map((user) => sendPushToUser(user.id, "Lead επιστράφηκε στο Pool", "Ένα lead επέστρεψε στο αδιάθετο pool μετά από 24 ώρες χωρίς επικοινωνία.", { type: "lead_inactivity_reallocated", agencyId })));
  }));
});