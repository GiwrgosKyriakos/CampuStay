...
exports.aggregateAgency = aggregateAgency;
const firestore_1 = require("firebase-admin/firestore");
const scheduler_1 = require("firebase-functions/v2/scheduler");
const pipelineConfig_1 = require("../lib/pipelineConfig");
const SOURCES = ["spitogatos", "xe_gr", "meta_ads", "google_ads", "agency_website", "referral", "walk_in", "signboard", "other"];
const EVENT_TYPES = ["listing_view", "lead_inquiry", "showing_conducted", "offer_submitted", "deal_stage_changed", "deal_closed", "deal_lost"];
const LOST_REASONS = ["price_dispute", "legal_defect", "competitor_won", "buyer_withdrew", "owner_cancelled", "financial_issue"];
...
function dealForecastValue(deal, config) {
    const stage = stringValue(deal.pipelineStage || deal.status).toLowerCase();
    if (["closed", "closed_won", "deal_closed", "lost", "closed_lost", "cancelled", "canceled"].includes(stage))
        return 0;
    ...
    return Math.max(0, commission) * (0, pipelineConfig_1.getProbabilityForStage)(stageKey, config);
}
function sourceValue(value) {
    const source = stringValue(value).toLowerCase().replace(/[ -]/g, "_");
    const legacyMap = { xe: "xe_gr", social_ads: "meta_ads", google: "google_ads", website: "agency_website", open_house: "other", openhouse: "other", yard_sign: "signboard" };
    const normalized = legacyMap[source] ?? source;
    return SOURCES.includes(normalized) ? normalized : "other";
}
function lostReasonValue(value) {
    return LOST_REASONS.includes(stringValue(value)) ? stringValue(value) : "other";
}
function periodDefinitions(now) {
...
function expectedCloseAt(deal, now) {
...
function periodKey(timestamp, granularity) {
...
function buildRevenueSeries(events, granularity) {
    const result = {};
    events.filter((event) => event.eventType === "deal_closed").forEach((event) => {
        ...
        if (event.transactionType === "sale")
            point.saleCommission += amount;
        else
            point.rentCommission += amount;
        result[key] = point;
    });
    return result;
}
function buildFunnelAnalytics(funnel) {
...
function buildAgentMetrics(users, apartments, appointments, allEvents, period) {
...
async function aggregateAgency(agencyId, now) {
    ...
    const pipelineConfig = await (0, pipelineConfig_1.getAgencyPipelineConfig)(agencyId);
    const events = eventsSnapshot.docs.map((document) => document.data()).filter((event) => EVENT_TYPES.includes(event.eventType));
    ...
    await Promise.all(periodDefinitions(now).map(async (period) => {
        ...
        const leadSource = (event) => sourceValue(event.source ?? leadsById.get(stringValue(event.leadId))?.source ?? leadsById.get(stringValue(event.leadId))?.leadSource);
        const leadCountsBySource = Object.fromEntries(SOURCES.map((source) => [source, periodEvents.filter((event) => event.eventType === "lead_inquiry" && leadSource(event) === source).length]));
        const attributedRevenueBySource = Object.fromEntries(SOURCES.map((source) => [source, periodEvents.filter((event) => event.eventType === "deal_closed" && leadSource(event) === source).reduce((sum, event) => sum + Math.max(0, numberValue(event.amount)), 0)]));
        const campaignSpendBySource = Object.fromEntries(SOURCES.map((source) => [source, spends.filter((spend) => sourceValue(spend.source) === source && (period.id === "all" || (typeof (spend.month ?? spend.period) === "string" && (spend.month ?? spend.period) >= new Date(period.start).toISOString().slice(0, 7) && (spend.month ?? spend.period) <= new Date(period.end).toISOString().slice(0, 7)))).reduce((sum, spend) => sum + Math.max(0, numberValue(spend.spendAmount ?? spend.spentAmount ?? spend.amount)), 0)]));
        ...
        const weightedForecast = deals.reduce((forecast, deal) => {
            ...
            if (closeAt <= now.getTime() + 30 * 86_400_000)
                forecast.next30Days += value;
            if (closeAt <= now.getTime() + 60 * 86_400_000)
                forecast.next60Days += value;
            return forecast;
