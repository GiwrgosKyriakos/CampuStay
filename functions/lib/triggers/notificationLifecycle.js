...
function offerPayload(data, offerId, action) {
...
function offerRecipients(data, response) {
...
async function notifyApprovedOffer(eventData, conversationId, offerId) {
    if (!eventData)
        return;
...
exports.onListingDocumentsUpdated = (0, firestore_2.onDocumentUpdated)("apartments/{apartmentId}", async (event) => {
    ...
    if (beforeDocuments === afterDocuments)
        return;
    const beforeDocumentMap = before.documents && typeof before.documents === "object" ? before.documents : {};
    const afterDocumentMap = after.documents && typeof after.documents === "object" ? after.documents : {};
...
function contractPayload(data, contractId, action) {
    const chatId = stringValue(data.chatRoomId);
    return {
        type: "document_required",
...
async function checklistRoleRecipients(role, deal) {
    if (role === "client")
        return [stringValue(deal.clientId)].filter(Boolean);
    if (role === "owner")
        return [stringValue(deal.ownerId || deal.hostId)].filter(Boolean);
    if (role === "broker")
        return dealBrokerUsers(deal);
    return agencyReviewers(stringValue(deal.agencyId));
}
function checklistPayload(type, title, body, dealId, clientId, itemId, action) {
    return {
        type,
        title,
        body,
...
exports.onChecklistItemUpdated = (0, firestore_2.onDocumentUpdated)("deals/{dealId}/checklist/{itemId}", async (event) => {
    ...
    if (after.status !== "verified" || before.status === "verified")
        return;
    await notifyUsers(await checklistRoleRecipients(stringValue(after.assignedToRole), deal), checklistPayload("document_verified", "Το έγγραφο εγκρίθηκε", `Το έγγραφο ${title} εγκρίθηκε.`, event.params.dealId, stringValue(deal.clientId), itemId, "view_checklist_document"));
...
//# sourceMappingURL=notificationLifecycle.js.map