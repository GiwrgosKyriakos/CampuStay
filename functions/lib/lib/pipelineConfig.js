...
exports.DEFAULT_AGENCY_PIPELINE_CONFIG = void 0;
exports.normalizeAgencyPipelineConfig = normalizeAgencyPipelineConfig;
exports.getAgencyPipelineConfig = getAgencyPipelineConfig;
exports.getProbabilityForStage = getProbabilityForStage;
const firestore_1 = require("firebase-admin/firestore");
const DEFAULT_AGENCY_PIPELINE_CONFIG = {
...
exports.DEFAULT_AGENCY_PIPELINE_CONFIG = DEFAULT_AGENCY_PIPELINE_CONFIG;
function numberInRange(value, fallback, maximum) {
...
function normalizeAgencyPipelineConfig(value) {
    const data = value ?? {};
...
async function getAgencyPipelineConfig(agencyId) {
    const snapshot = await (0, firestore_1.getFirestore)().doc(`agencies/${agencyId}/settings/pipeline_config`).get();
    return normalizeAgencyPipelineConfig(snapshot.exists ? snapshot.data() : undefined);
}
function getProbabilityForStage(stage, config) {
    if (["closed_won", "deal_closed", "contract_completed", "contract_completed_100"].includes(String(stage)))
        return config.stageProbabilities.contract_completed_100;
    if (["negotiation_agreement", "preliminary_signed", "preliminary_signed_90"].includes(String(stage)))
        return config.stageProbabilities.preliminary_signed_90;
    if (["docs_review"].includes(String(stage)))
        return config.stageProbabilities.docs_review;
    if (["offer", "offer_made"].includes(String(stage)))
        return config.stageProbabilities.offer_made;
    if (["showing_scheduled"].includes(String(stage)))
        return config.stageProbabilities.showing_scheduled;
    if (["showing_completed", "showing_planned"].includes(String(stage)))
        return config.stageProbabilities.showing_completed;
