...
import { generateContractPdf } from "@/src/services/pdfGenerator";
...
export default function SignContractModal({ visible, draft, contractId, signerId: signerIdProp, onClose, onCreated, onCompleted }: SignContractModalProps) {
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      try {
        ...
        const deposit = resolvedContract.contractPayload.holdingDepositAmount;
        ...
        setHouseRulesText(houseRules && typeof houseRules === "object" && typeof houseRules.houseRules === "string" ? houseRules.houseRules : "");
  ...
  const previewHtml = useMemo(() => {
    ...
    const previewPayload = {
      ...
      ...(contract.contractType === "holding_deposit_viewing" ? { holdingDepositAmount: Number(holdingDepositAmount.replace(",", ".")) || 0 } : {}),
      ...(contract.contractType === "roommate_agreement" ? { houseRulesConfig: { ...contract.contractPayload.houseRulesConfig, houseRules: houseRulesText.trim() } } : {}),
    };
  ...
  }, [agency, contract, holdingDepositAmount, houseRulesText, idBackUrl, idFrontUrl, participants, property, signerAfm, signerIdCardNumber, signerId]);
  ...
  const handleFinalize = async () => {
    if (!contract || !currentSigner || !locationCoords || !signatureData.trim() || !idFrontUrl || !idBackUrl || !signerIdCardNumber.trim() || (otpRequired && !otpVerified) || isFinalizing) return;
    ...
    try {
      ...
      const html = buildContractHtml({ document: { ...contract, signers: nextSigners }, agency: agency!, property, participants: signedParticipants });
      const generatedPdf = await generateContractPdf(html);
  ...
  const handleWhatsAppShare = async () => {
    ...
    const message = `Το υπογεγραμμένο αντίγραφο είναι διαθέσιμο εδώ: ${successContract.pdfStorageUrl || ""}`;
    await Linking.openURL(`whatsapp://send?phone=${encodeURIComponent(sharePhone)}&text=${encodeURIComponent(message)}`).catch(() => setErrorText(t("esign.errors.whatsapp")));
  };

  const contractPayloadValid = contract?.contractType === "holding_deposit_viewing"
    ? Number(holdingDepositAmount.replace(",", ".")) > 0
    : contract?.contractType === "roommate_agreement"
      ? houseRulesText.trim().length > 0
      : true;
  ...
  const canContinueFromStep = step === 1
    ...
    : step === 2
      ? idFrontUrl.length > 0 && idBackUrl.length > 0
      : step === 3
        ? !otpRequired || otpVerified
        : Boolean(signatureData && locationCoords && idFrontUrl && idBackUrl && signerIdCardNumber.trim() && (!otpRequired || otpVerified));
  ...
  const goNext = async () => {
    ...
    if (step === 1 && hasEditablePayload && contract && !contract.signers.some((signer) => signer.signatureBase64.trim())) {
      ...
      try {
        const payload = {
          ...contract.contractPayload,
          ...(contract.contractType === "holding_deposit_viewing" ? { holdingDepositAmount: Number(holdingDepositAmount.replace(",", ".")) } : {}),
          ...(contract.contractType === "roommate_agreement" ? { houseRulesConfig: { ...contract.contractPayload.houseRulesConfig, houseRules: houseRulesText.trim() } } : {}),
        };
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
            ...
            <ScrollView style={styles.body} contentContainerStyle={styles.bodyContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
              ...
              {step === 3 ? (
                <>
                  ...
                  <View style={[styles.switchRow, { borderColor: colors.border, backgroundColor: colors.surfaceSecondary }]}><View style={styles.switchCopy}><Text style={[styles.label, { color: colors.onSurface }]}>{t("esign.requireOtp")}</Text><Text style={[styles.sectionHint, { color: colors.onSurfaceTertiary }]}>{t("esign.otpOptional")}</Text></View><Switch value={otpRequired} onValueChange={setOtpRequired} trackColor={{ false: colors.surfaceTertiary, true: colors.brandTertiary }} thumbColor={otpRequired ? colors.brand : colors.onSurfaceTertiary} /></View>
                  {otpRequired ? <>
      ...
      {contract ? <IdCameraCapture visible={cameraVisible} contractId={contract.id} signerId={signerId} frontUrl={idFrontUrl} backUrl={idBackUrl} onUploaded={(side, url) => { if (side === "front") setIdFrontUrl(url); else setIdBackUrl(url); }} onClose={() => setCameraVisible(false)} /> : null}
    </Modal>
  );
}

const styles = StyleSheet.create({
  backdrop: { flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(5,33,40,0.72)" },
  card: { maxHeight: "94%", borderTopLeftRadius: radius.lg, borderTopRightRadius: radius.lg, borderWidth: 1, overflow: "hidden" },
