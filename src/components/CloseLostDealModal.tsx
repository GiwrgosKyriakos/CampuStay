...
const LOSS_OPTIONS: { value: LostDealReason; label: string; icon: keyof typeof Ionicons.glyphMap }[] = [
  { value: "price_too_high", label: "Υψηλή Τιμή / Ασυμφωνία Ιδιοκτήτη", icon: "pricetag-outline" },
  { value: "legal_tax_issues", label: "Τεχνικό / Νομικό Κόλλημα", icon: "document-text-outline" },
  { value: "competitor_won", label: "Επιλογή Άλλου Ακινήτου", icon: "home-outline" },
  { value: "competitor_won", label: "Αγορά μέσω άλλου γραφείου", icon: "business-outline" },
...
export default function CloseLostDealModal({ visible, apartmentTitle, onClose, onConfirm }: { visible: boolean; apartmentTitle: string; onClose: () => void; onConfirm: (reason: LostDealReason) => void }) {
  const { colors } = useTheme();
  const [selectedOptionIndex, setSelectedOptionIndex] = useState<number | null>(null);
  useEffect(() => {
    if (visible) setSelectedOptionIndex(null);
  }, [visible]);
