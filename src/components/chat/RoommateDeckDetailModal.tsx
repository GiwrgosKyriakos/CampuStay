...
import { localizeGender, localizeLifestyle, localizeQuizAnswer, type QuizQuestionKey } from "@/src/utils/localizeData";
...
export default function RoommateDeckDetailModal({ visible, profile, onClose }: { visible: boolean; profile: SharedProfileMessageMetadata | null; onClose: () => void }) {
  ...
  const localizeQuizValue = (key: string, value: unknown) => (/^q\d+$/.test(key) ? localizeQuizAnswer(String(value), key as QuizQuestionKey) : localizeLifestyle(String(value)));

  return (
    <BaseBottomSheet visible={visible} onClose={onClose} maxHeight="90%">
      <View style={styles.contentWrap}>
        <View style={styles.header}>
          <Text style={[styles.title, { color: colors.onSurface }]}>Προφίλ συγκάτοικου</Text>
          <Pressable onPress={onClose}><Ionicons name="close-outline" size={24} color={colors.onSurface} /></Pressable>
        </View>
        <View style={styles.content}>
          ...
          <Text style={[styles.name, { color: colors.onSurface }]}>{data.fullName}</Text>
          {data.compatibilityScore != null ? <View style={[styles.matchBadge, { backgroundColor: colors.brandTertiary }]}><Ionicons name="sparkles-outline" size={16} color={colors.brand} /><Text style={[styles.matchText, { color: colors.brand }]}>{data.compatibilityScore}% Match</Text></View> : null}
          <View style={styles.metaGrid}>
            {data.age ? <Text style={[styles.meta, { color: colors.onSurfaceTertiary }]}>{data.age} ετών</Text> : null}
            {data.budget ? <Text style={[styles.meta, { color: colors.onSurfaceTertiary }]}>€{data.budget}/μήνα</Text> : null}
            {data.gender ? <Text style={[styles.meta, { color: colors.onSurfaceTertiary }]}>{localizeGender(data.gender)}</Text> : null}
            {data.occupation ? <Text style={[styles.meta, { color: colors.onSurfaceTertiary }]}>{localizeLifestyle(data.occupation)}</Text> : null}
          </View>
          {data.program || data.university ? <Text style={[styles.meta, { color: colors.onSurfaceTertiary }]}>{[data.program, data.university].filter(Boolean).join(" · ")}</Text> : null}
          {data.bio ? <View style={styles.section}><Text style={[styles.sectionTitle, { color: colors.onSurface }]}>Σχετικά</Text><Text style={[styles.body, { color: colors.onSurfaceTertiary }]}>{data.bio}</Text></View> : null}
          {data.lifestyleBadges?.length ? <View style={styles.section}><Text style={[styles.sectionTitle, { color: colors.onSurface }]}>Lifestyle</Text><View style={styles.badges}>{data.lifestyleBadges.map((badge) => <View key={badge} style={[styles.badge, { backgroundColor: colors.brandTertiary }]}><Text style={[styles.badgeText, { color: colors.onBrandTertiary }]}>{localizeLifestyle(badge)}</Text></View>)}</View></View> : null}
          {quizAnswers.length ? <View style={styles.section}><Text style={[styles.sectionTitle, { color: colors.onSurface }]}>Ανάλυση συμβατότητας</Text>{quizAnswers.map(([key, value]) => <View key={key} style={styles.detailRow}><Text style={[styles.detailKey, { color: colors.onSurfaceTertiary }]}>{key}</Text><Text style={[styles.body, { color: colors.onSurface }]}>{localizeQuizValue(key, value)}</Text></View>)}</View> : null}
          {data.hardCriteria ? <View style={styles.section}><Text style={[styles.sectionTitle, { color: colors.onSurface }]}>Hard criteria</Text>{Object.entries(data.hardCriteria).map(([key, value]) => <Text key={key} style={[styles.body, { color: colors.onSurfaceTertiary }]}>{key}: {String(value)}</Text>)}</View> : null}
        </View>
      </View>
    </BaseBottomSheet>
  );
