...
export default function OpenHouseScannerModal({ visible, agencyId, apartmentId, apartmentTitle, brokerId, onClose, onRegistered }: { visible: boolean; agencyId: string; apartmentId: string; apartmentTitle: string; brokerId: string; onClose: () => void; onRegistered?: () => void }) {
  ...
  const reset = () => { setName(""); setPhone(""); setEmail(""); setBudget(""); setSource("other"); };
  const submit = async () => {
    if (!name.trim() || saving) return;
    setSaving(true);
    try {
      await registerOpenHouseLead({ agencyId, apartmentId, apartmentTitle, clientName: name, phone, email, budget: budget.trim() ? Number(budget) : undefined, source, registeredByBrokerId: brokerId });
      reset();
      onRegistered?.();
      onClose();
    } catch (error) {
...
const createStyles = (colors: ReturnType<typeof useTheme>["colors"]) => StyleSheet.create({
  ...
  sourceLabel: { fontFamily: fonts.semibold, fontSize: fontSize.xs, color: colors.onSurfaceTertiary },
