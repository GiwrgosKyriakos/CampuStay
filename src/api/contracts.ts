...
export async function getContractDocument(contractId: string): Promise<DigitalContractDocument | null> {
  ...
  return snapshot.exists() ? mapContract(snapshot.id, snapshot.data() as Record<string, unknown>) : null;
}
...
export async function recordContractSignature(params: {
  ...
  return mapContract(params.contractId, result.data);
}
...
export async function verifySigningOtp(contractId: string, signerId: string, code: string): Promise<{ verified: boolean; verifiedAt: number }> {
  const callable = httpsCallable<{ contractId: string; signerId: string; code: string }, { verified: boolean; verifiedAt: number }>(firebaseFunctions, "verifySigningOtp");
