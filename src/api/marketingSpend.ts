...
export async function saveCampaignSpend(input: { agencyId: string; source: StandardLeadSource; month: string; spendAmount: number; recordedBy: string; recordedByName?: string }): Promise<CampaignSpend> {
  ...
  const reference = await addDoc(collection(db, "agencies", input.agencyId, "campaign_spends"), {
    ...
    recordedByName: input.recordedByName?.trim() || "Μεσίτης",
  });
  return { id: reference.id, agencyId: input.agencyId, source: input.source, month: input.month, spendAmount: input.spendAmount, currency: "EUR", recordedAt, recordedBy: input.recordedBy, recordedByName: input.recordedByName?.trim() || "Μεσίτης" };
}